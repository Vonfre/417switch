...
import { ScienceProviderForm } from "@/components/science/ScienceProviderForm";
...
export function ProviderForm(props: ProviderFormProps) {
  if (props.appId === "science") {
    return <ScienceProviderForm {...props} />;
  }
...
function ProviderFormFull({
  ...
  const isClaudeLike = appId === "claude" || appId === "science";
  ...
  const supportsFullUrl = isClaudeLike || appId === "codex";
  ...
  const [localApiKeyField, setLocalApiKeyField] = useState<ClaudeApiKeyField>(
    () => {
      if (!isClaudeLike) return "ANTHROPIC_AUTH_TOKEN";
  ...
  const {
    ...
    category,
    appType: appId,
    apiKeyField: isClaudeLike ? localApiKeyField : undefined,
  });
  ...
  const {
    ...
    selectedPresetId: isClaudeLike ? selectedPresetId : null,
    presetEntries: isClaudeLike ? presetEntries : [],
    settingsConfig: form.getValues("settingsConfig"),
    onConfigChange: handleSettingsConfigChange,
  });
  ...
  const shouldApplyLocalProxyRequestOverrides =
    (isClaudeLike || appId === "codex") && category !== "official";
  ...
  const handleSubmit = async (values: ProviderFormData) => {
    ...
    // 模板变量未填：A 类（空值）
    if (isClaudeLike && templateValueEntries.length > 0) {
    ...
    if (category !== "official" && category !== "cloud_provider") {
      if (isClaudeLike) {
        if (!isCodexOauthProvider && !isXaiOauthProvider && !baseUrl.trim()) {
          const message = t("providerForm.endpointRequired", {
        ...
        if (
          ...
          !isXaiOauthProvider &&
          !apiKey.trim() &&
          !(appId === "science" && isLoopbackEndpoint(baseUrl))
        ) {
          issues.push(
    ...
    if (
      ...
      toast.error(
        t("providerForm.scienceModelRequired", {
          defaultValue: "Claude Science 至少需要配置一个模型",
        }),
      );
      return;
    }
    ...
    const nextMeta: ProviderMeta = {
      ...
      pricingModelSource:
        pricingConfig.enabled && pricingConfig.pricingModelSource !== "inherit"
          ...
          : undefined,
      apiFormat:
        isClaudeLike && category !== "official"
          ? isXaiOauthProvider
            ? "openai_responses"
            : localApiFormat
  ...
  const handlePresetChange = (value: string) => {
    ...
    if (value === "custom") {
      ...
      // Claude Science shares the Claude-compatible provider form, but its
      // custom provider must start from the native Anthropic wire format.
      // Otherwise switching from an OpenAI-compatible preset leaves the form
        ...
        >
          ...
          {isClaudeLike && (
            <ClaudeFormFields
              ...
              // Science needs an editable API endpoint just like Claude Code.
              // Its endpoint-management commands still use AppType and do not
              // yet have an isolated Science namespace, so keep only the plain
              // input enabled to avoid writing Science endpoints into Claude.
              showEndpointTools={appId !== "science"}
