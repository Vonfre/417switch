...
const APP_CONFIG: Array<{
  ...
  { id: "science", icon: "science", name: "Claude Science" },
];
...
export function AppVisibilitySettings({
  ...
  const visibleProviderCount = APP_CONFIG.filter(
  ...
  const handleToggle = (appId: AppId) => {
    ...
    if (isCurrentlyVisible && visibleProviderCount <= 1) return;
    ...
    <section className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1 rounded-md border border-border-default bg-background p-1">
        {APP_CONFIG.map((app) => {
          ...
          const isDisabled = isVisible && visibleProviderCount <= 1;
          const name = app.name ?? t(app.nameKey!);
