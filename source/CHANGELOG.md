...
- Open the healthy Claude Science login surface through macOS Launch Services
  directly, avoiding a pending opener call that could leave 417Switch showing
  “starting” after the daemon was already ready.
- Poll Science status while startup is pending and expose the Open action as
  soon as the managed daemon is confirmed healthy.
...
its independent identity, updater channel, and Claude Science integration. The
merge adds Pi support, multi-account Codex management, hosted WebSearch and
Alpha Search routing, provider-form and IME improvements, and the upstream
backup/sync hardening. A new schema v18 reconciliation migration safely handles
the v17 version collision between earlier 417Switch Science metadata and the
upstream session-usage deduplication ledger.
...
- Expanded the Claude Science Simplified Chinese patch across the dashboard, General settings, connector filters and health notices, account metadata, licensing, contact-email, diagnostics, update controls, dynamic project actions, relative times, and accessibility labels.
- Claude Science automatic background updates are enabled again. 417Switch no longer launches the managed daemon with `--no-auto-update`; updated Science runtimes remain automatically detected and receive a freshly generated Chinese asset patch after restart.
