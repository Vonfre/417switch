...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".417switch").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".417switch");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.417switch/settings.json
