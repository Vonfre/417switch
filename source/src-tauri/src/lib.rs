...
mod science {
    ...
    pub async fn status(_state: &AppState) -> ScienceStatus {
        ScienceStatus {
            supported: false,
            installed: false,
            running: false,
    ...
    pub async fn start(
        ...
        Err("Claude Science 集成当前仅支持 macOS".into())
    }

    pub async fn open(_app: &tauri::AppHandle) -> Result<String, String> {
        Err("Claude Science 集成当前仅支持 macOS".into())
    }
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/cc-switch.log）
            {
                ...
                log::info!("=== 417Switch v{} started ===", env!("CARGO_PKG_VERSION"));
            }
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only the first 417Switch deep link
                        }
                    }
                }
            });
            ...
            tauri::async_runtime::spawn(async move {
                ...
                // Claude Science daemon survives a 417Switch restart, but its
                // ANTHROPIC_BASE_URL still points at 417Switch's local route.
                // Restore that route automatically so an already-open Science
                // window never becomes disconnected after the main app restarts.
                match crate::science::restore_proxy_if_running(&state).await {
                    Ok(true) => log::info!("Claude Science 本地 Provider 路由已恢复"),
                    Ok(false) => {}
                    Err(e) => log::warn!("恢复 Claude Science 本地 Provider 路由失败: {e}"),
                }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Claude Science integration (reuses the current Claude provider)
            commands::get_science_status,
            commands::start_science,
            commands::stop_science,
            commands::open_science,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 switch417://...）
...
pub async fn cleanup_before_exit(app_handle: &tauri::AppHandle) {
    // Science routes inference through the local proxy. Stop the isolated,
    // 417Switch-managed instance before tearing that proxy down so a normal
    // app exit cannot leave an unusable background listener behind.
    #[cfg(target_os = "macos")]
    {
        if let Err(e) = science::stop().await {
            log::error!("退出时停止 Claude Science 失败: {e}");
        }
    }
