...
impl ProxyServer {
    ...
    fn build_router(&self) -> Router {
        Router::new()
            ...
            // Claude Science uses the current Claude provider through an isolated
            // base URL so its model catalog cannot be confused with Codex.
            .route(
                "/science/v1/messages",
                post(handlers::handle_science_messages),
            )
            .route("/science/v1/models", get(handlers::handle_science_models))
