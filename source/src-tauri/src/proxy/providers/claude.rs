...
/// Normalize an OpenAI-compatible base root in the same way CSSwitch does.
/// Users may paste an origin, a versioned root, or a complete endpoint; the
/// proxy must still produce exactly one `/responses` or `/chat/completions`
/// suffix and add `/v1` only when the root has no version segment.
fn normalize_openai_compatible_base(base: &str) -> String {
...
fn ends_with_openai_version_segment(base: &str) -> bool {
...
fn build_openai_compatible_url(base: &str, endpoint: &str) -> Option<String> {
    let (path, query) = endpoint
        .split_once('?')
        .map_or((endpoint, None), |(path, query)| (path, Some(query)));
    let suffix = match path.trim_end_matches('/') {
        "/v1/responses" => "/responses",
        "/v1/chat/completions" => "/chat/completions",
        _ => return None,
    };

    let mut root = normalize_openai_compatible_base(base);
    if !ends_with_openai_version_segment(&root) {
        root.push_str("/v1");
    }
...
impl ProviderAdapter for ClaudeAdapter {
    ...
    fn build_url(&self, base_url: &str, endpoint: &str) -> String {
        ...
        // OpenAI-compatible endpoints use a base root contract: `host`,
        // `host/v1`, and even a pasted full endpoint must all resolve to one
        // canonical inference URL. This is especially important for Claude
        // Science providers using the Responses adapter.
        if let Some(url) = build_openai_compatible_url(base_url, endpoint) {
            return url;
        }
...
mod tests {
    ...
    #[test]
    fn openai_responses_base_root_auto_appends_responses_like_csswitch() {
        let adapter = ClaudeAdapter::new();
        for (base, expected) in [
            (
                "http://45.205.25.145:3010",
                "http://45.205.25.145:3010/v1/responses",
            ),
            (
                "http://45.205.25.145:3010/v1",
                "http://45.205.25.145:3010/v1/responses",
            ),
            (
                "http://45.205.25.145:3010/v1/responses",
            ...
            (
                ...
                "https://gateway.example/v2/responses",
            ),
        ] {
            assert_eq!(adapter.build_url(base, "/v1/responses"), expected);
        }
        assert_eq!(
            adapter.build_url(
                "http://45.205.25.145:3010/v1/responses",
            ...
            "http://45.205.25.145:3010/v1/responses?trace=1"
        );
    }

    #[test]
    fn copilot_unversioned_chat_endpoint_keeps_existing_contract() {
        let adapter = ClaudeAdapter::new();
        assert_eq!(
            adapter.build_url("https://api.githubcopilot.com", "/chat/completions"),
