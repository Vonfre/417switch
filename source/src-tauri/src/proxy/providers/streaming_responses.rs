                    ...
                    while let Some(block) = take_sse_block(&mut buffer) {
                        ...
                        match event_name {
                            ...
                            "response.created" => {
                                ...
                                // A malformed compatible gateway may emit response.created
                                // after a delta. Never send a second message_start: Anthropic
                                // stream consumers initialize their content accumulator here.
                            ...
                            "response.output_item.added" => {
                                if let Some(item) = data.get("item") {
                                    ...
                                    if item_type == "function_call" {
                                        ...
                                        // Reserving an Anthropic content index without starting
                                        // the block creates a gap (for example the following text
                                        // starts at index 1 while content is still empty). Claude
                                        // Science's parser indexes into message.content and fails
                                        // with `content.push` in that situation. Start the thinking
                                        // block as soon as the Responses reasoning item is added.
                                        if !open_indices.contains(&index) {
                                            yield Ok(anthropic_sse(
                                                "content_block_start",
                                                &json!({
...
mod tests {
    ...
    #[tokio::test]
    async fn test_message_start_initializes_anthropic_content_accumulator() {
        let input = concat!(
            "event: response.created\n",
            "data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_content\",\"model\":\"gpt-5.6\"}}\n\n",
            "event: response.output_text.delta\n",
        ...
        let merged = convert_stream_text(input).await;
        let start: Value = merged
            .split("\n\n")
            .find_map(|block| {
                let payload = block.lines().find_map(|line| line.strip_prefix("data: "))?;
        ...
        assert_eq!(start.pointer("/message/content"), Some(&json!([])));
        assert_eq!(start.pointer("/message/stop_reason"), Some(&Value::Null));
        assert_eq!(start.pointer("/message/stop_sequence"), Some(&Value::Null));
    }

    #[tokio::test]
    async fn test_reasoning_reservation_does_not_create_content_index_gap() {
        let input = concat!(
            "event: response.created\n",
            "data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_gap\",\"model\":\"gpt-5.6\"}}\n\n",
            "event: response.output_item.added\n",
            "data: {\"type\":\"response.output_item.added\",\"output_index\":0,\"item\":{\"id\":\"rs_gap\",\"type\":\"reasoning\",\"summary\":[]}}\n\n",
            "event: response.content_part.added\n",
            "data: {\"type\":\"response.content_part.added\",\"item_id\":\"msg_gap\",\"output_index\":1,\"content_index\":0,\"part\":{\"type\":\"output_text\",\"text\":\"\"}}\n\n",
            "event: response.output_text.delta\n",
            "data: {\"type\":\"response.output_text.delta\",\"item_id\":\"msg_gap\",\"output_index\":1,\"content_index\":0,\"delta\":\"answer\"}\n\n",
            "event: response.output_item.done\n",
            "data: {\"type\":\"response.output_item.done\",\"output_index\":0,\"item\":{\"id\":\"rs_gap\",\"type\":\"reasoning\",\"summary\":[],\"encrypted_content\":\"opaque\"}}\n\n",
            "event: response.completed\n",
            "data: {\"type\":\"response.completed\",\"response\":{\"status\":\"completed\"}}\n\n"
        );
        let merged = convert_stream_text(input).await;
        let start_indices: Vec<u64> = merged
            .split("\n\n")
            .filter_map(|block| {
                let payload = block.lines().find_map(|line| line.strip_prefix("data: "))?;
                let event: Value = serde_json::from_str(payload).ok()?;
                (event.get("type").and_then(Value::as_str) == Some("content_block_start"))
                    .then(|| event.get("index").and_then(Value::as_u64))
                    .flatten()
            })
            .collect();
