...
impl RequestForwarder {
    ...
    async fn forward_with_retry_inner(
        ...
        // The protocol adapter and provider namespace are separate concerns.
        // Claude Science speaks Anthropic/Claude protocol but owns an isolated
        // `science` provider bucket and circuit-breaker state.
        let app_type_str = routing_app_type;
...
fn inspect_responses_start_event(block: &str) -> Option<Result<(), ProxyError>> {
    ...
    match event {
        ...
        // `response.created` is productive for the downstream converter: it is
        // immediately translated to Anthropic `message_start`. Holding it back
        // while waiting for text/tool output makes reasoning-heavy providers look
        // disconnected to Science and can trigger "Connection issue — retrying".
        // Once a syntactically valid lifecycle event arrives, commit/replay the
        // stream and let the converter surface any later terminal error.
        "response.created" | "response.in_progress" | "response.queued" => Some(Ok(())),
        "" => None,
...
mod tests {
    ...
    fn responses_stream_start_semantic_failure_is_retryable() {
        ...
        assert!(matches!(
            inspect_responses_start_event(created),
            Some(Ok(()))
        ));

        let reasoning_item = concat!(
            "event: response.output_item.added\n",
            "data: {\"type\":\"response.output_item.added\",\"item\":{\"type\":\"reasoning\",\"id\":\"rs_1\"}}"
        );
        assert!(matches!(
            inspect_responses_start_event(reasoning_item),
            Some(Ok(()))
        ));
