...
/// 获取 417Switch 应用配置目录路径 (~/.417switch)。
///
/// 该 fork 必须与正式版 CC Switch 隔离，禁止自动回退或迁移
/// `~/.cc-switch`，否则两个应用会共享数据库和运行状态。
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    let default_dir = get_home_dir().join(".417switch");
    ...
    #[cfg(windows)]
    {
        ...
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                ...
                if !trimmed.is_empty() {
                    let legacy_dir = PathBuf::from(trimmed).join(".417switch");
