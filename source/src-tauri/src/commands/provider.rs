...
fn science_provider_has_managed_route(provider: &Provider) -> bool {
    provider.is_codex_oauth()
        ...
        || provider
            ...
            .is_some_and(|provider_type| provider_type == "github_copilot")
}

fn science_provider_has_api_key(provider: &Provider) -> bool {
    let settings = match provider.settings_config.as_object() {
        Some(settings) => settings,
        None => return false,
    };
    let env = settings.get("env").and_then(serde_json::Value::as_object);
    [
        env.and_then(|values| values.get("ANTHROPIC_AUTH_TOKEN")),
        env.and_then(|values| values.get("ANTHROPIC_API_KEY")),
        settings.get("apiKey"),
...
fn science_provider_has_model(provider: &Provider) -> bool {
    provider
        .settings_config
        .get("env")
        .and_then(serde_json::Value::as_object)
...
pub(crate) fn validate_science_provider(provider: &Provider) -> Result<(), String> {
    if !provider.settings_config.is_object() {
        return Err("Claude Science 配置必须是 JSON 对象".to_string());
    }
    if science_provider_base_url(provider).is_none()
        && !science_provider_has_managed_route(provider)
    {
        return Err("Claude Science Provider 缺少 API 端点；请填写 ANTHROPIC_BASE_URL".to_string());
    }
    if !science_provider_has_model(provider) {
        return Err("Claude Science Provider 至少需要配置一个模型".to_string());
    }
    if !science_provider_has_api_key(provider) && !science_provider_has_managed_route(provider) {
        return Err(
            "Claude Science Provider 缺少 API Key；请填写 ANTHROPIC_AUTH_TOKEN".to_string(),
        );
    }
    Ok(())
}

fn science_providers(state: &AppState) -> Result<IndexMap<String, Provider>, String> {
    state
        .db
        .get_all_providers(SCIENCE_APP_ID)
        .map_err(|e| e.to_string())
}

/// First-run migration for builds that previously reused Claude Code's current
/// provider directly. Providers are cloned once into an isolated `science`
/// bucket; all later edits and switches are independent.
pub(crate) fn ensure_science_provider_seed(state: &AppState) -> Result<(), String> {
    if !science_providers(state)?.is_empty() {
        return Ok(());
    }
    ...
    for provider in claude_providers.values() {
        state
            .db
            .save_provider(SCIENCE_APP_ID, provider)
            .map_err(|e| e.to_string())?;
    }

    let preferred =
        crate::settings::get_effective_current_provider(state.db.as_ref(), &AppType::Claude)
            .map_err(|e| e.to_string())?
            .filter(|id| claude_providers.contains_key(id))
            .or_else(|| claude_providers.keys().next().cloned());
    if let Some(id) = preferred {
        state
            .db
            .set_current_provider(SCIENCE_APP_ID, &id)
            .map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
pub fn get_providers(
    ...
    if app == SCIENCE_APP_ID {
        ensure_science_provider_seed(state.inner())?;
        return science_providers(state.inner());
    }
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
...
pub fn get_current_provider(state: State<'_, AppState>, app: String) -> Result<String, String> {
    if app == SCIENCE_APP_ID {
        ...
        return state
            .db
            .get_current_provider(SCIENCE_APP_ID)
...
pub async fn add_provider(
    ...
    if app == SCIENCE_APP_ID {
        ...
        validate_science_provider(&provider)?;
        state
            .db
            .save_provider(SCIENCE_APP_ID, &provider)
            .map_err(|e| e.to_string())?;
            ...
            .is_none()
        {
            state
                .db
                .set_current_provider(SCIENCE_APP_ID, &provider.id)
                .map_err(|e| e.to_string())?;
        }
        return Ok(true);
    }
...
pub async fn update_provider(
    ...
    if app == SCIENCE_APP_ID {
        ...
        if originalId.as_deref().is_some_and(|id| id != provider.id) {
            return Err("Claude Science Provider ID 不支持修改".to_string());
        }
        state
            .db
            .save_provider(SCIENCE_APP_ID, &provider)
...
pub fn delete_provider(
    ...
    if app == SCIENCE_APP_ID {
        ...
        if current.as_deref() == Some(id.as_str()) {
            return Err("无法删除当前正在使用的 Claude Science Provider".to_string());
        }
        state
            .db
            .delete_provider(SCIENCE_APP_ID, &id)
            .map(|_| true)
            .map_err(|e| e.to_string())
    } else {
        let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
...
pub async fn switch_provider(
    ...
    if app == SCIENCE_APP_ID {
        ...
        let provider = state
            .db
            .get_provider_by_id(&id, SCIENCE_APP_ID)
            .map_err(|e| e.to_string())?
            .ok_or_else(|| format!("Claude Science Provider {id} 不存在"))?;
        validate_science_provider(&provider)?;
        state
            .db
            .set_current_provider(SCIENCE_APP_ID, &id)
        ...
        let _ = app_handle.emit(
            "provider-switched",
            serde_json::json!({ "appType": SCIENCE_APP_ID, "providerId": id }),
        );
...
pub fn update_providers_sort_order(
    ...
    if app == SCIENCE_APP_ID {
        let mut providers = science_providers(state.inner())?;
...
mod science_provider_validation_tests {
    ...
    #[test]
    fn rejects_model_only_provider_without_upstream() {
        let provider = provider(json!({
            "env": { "ANTHROPIC_MODEL": "gpt-5.5" }
        }));
        let error = validate_science_provider(&provider).expect_err("missing endpoint");
        assert!(error.contains("ANTHROPIC_BASE_URL"));
    }

    #[test]
    fn rejects_loopback_upstream_without_static_token() {
        let provider = provider(json!({
            "env": {
        ...
        let error = validate_science_provider(&provider).expect_err("missing API key");
        assert!(error.contains("API Key"));
    }

    #[test]
    fn accepts_upstream_with_static_token() {
        let provider = provider(json!({
            "env": {
                "ANTHROPIC_BASE_URL": "http://127.0.0.1:9876",
        ...
        validate_science_provider(&provider).expect("authenticated gateway is complete");
    }

    #[test]
    fn rejects_upstream_without_any_model_mapping() {
        let provider = provider(json!({
