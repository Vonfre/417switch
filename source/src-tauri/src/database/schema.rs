...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS proxy_config (
            ...
            listen_port INTEGER NOT NULL DEFAULT 41721, enable_logging INTEGER NOT NULL DEFAULT 1,
        ...
        let _ = conn.execute(
            "ALTER TABLE proxy_config ADD COLUMN listen_port INTEGER NOT NULL DEFAULT 41721",
            [],
        );
        let _ = conn.execute(
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    17 => {
                        log::info!(
                            "迁移数据库从 v17 到 v18（兼容 417Switch Science 与上游会话账本）"
                        );
    ...
    fn migrate_v1_to_v2(conn: &Connection) -> Result<(), AppError> {
        ...
        if Self::table_exists(conn, "proxy_config")? {
            ...
            Self::add_column_if_missing(
                ...
                "proxy_config",
                "listen_port",
                "INTEGER NOT NULL DEFAULT 41721",
            )?;
            Self::add_column_if_missing(
                conn,
                "proxy_config",
    ...
    fn migrate_v13_to_v14(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute(
            "CREATE TABLE proxy_config_v14 (
                ...
                listen_port INTEGER NOT NULL DEFAULT 41721,
    ...
    /// v17 -> v18: reconcile the schema-version collision between 417Switch
    /// v3.19.x (Science Responses metadata) and upstream CC Switch v3.20.0
    /// (session usage deduplication). Both operations are idempotent so this
    /// safely upgrades databases created by either release line.
...
mod tests {
    ...
    #[test]
    fn migrate_v17_to_v18_reconciles_science_and_usage_schema_lines() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        Database::create_tables_on_conn(&conn)?;
        conn.execute_batch(
        ...
        let migrated: (i64, String) = conn.query_row(
            "SELECT
             ...
             FROM providers WHERE id = 'legacy-science'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
        assert_eq!(migrated, (1, "science_custom".to_string()));
        let opt_out: i64 = conn.query_row(
            "SELECT json_extract(meta, '$.codexCompatibleResponses')
             FROM providers WHERE id = 'science-opt-out'",
            [],
            |row| row.get(0),
        )?;
        assert_eq!(opt_out, 0);
